...
// Helper to safely access localStorage (protects against SSR and mocked window environments)
...
export default function HoverReceiver() {
  ...
  const [isVisualEditMode, setIsVisualEditMode] = useState(() => {
    ...
    if (storage) {
      try {
        const stored = storage.getItem(VISUAL_EDIT_MODE_KEY);
        return stored === "true";
      } catch (e) {
        // Ignore errors
      }
  ...
  useEffect(() => {
    ...
    if (storage) {
      try {
        storage.setItem(VISUAL_EDIT_MODE_KEY, String(isVisualEditMode));
      } catch (e) {}
    }
  }, [isVisualEditMode]);
      ...
      setTimeout(() => {
        ...
        if (storage) {
          try {
            ...
            const focusedData = storage.getItem(FOCUSED_ELEMENT_KEY);
            if (focusedData) {
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      if (hit) {
        ...
        if (focusedElementRef.current) {
          ...
          if (storage) {
            try {
              storage.removeItem(FOCUSED_ELEMENT_KEY);
            } catch (e) {}
          }
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "VISUAL_EDIT_MODE" && "active" in e.data) {
        ...
        if (!newMode) {
          ...
          if (storage) {
            try {
              storage.removeItem(VISUAL_EDIT_MODE_KEY);
              storage.removeItem(FOCUSED_ELEMENT_KEY);
            } catch (e) {}
          }
        }
    ...
    <>
      {isVisualEditMode && (
        <div className="fixed inset-0 pointer-events-none z-[999999]">
          {/* Hover boxes (for other elements with same ID) */}
          {hoverBoxes.map((box, i) => box && (
            <div
              key={`hover-${i}`}
              className="absolute border border-blue-400/50 bg-blue-400/5 rounded-sm transition-all duration-150 ease-out"
              style={{
                top: box.top,
                left: box.left,
                width: box.width,
            ...
            >
              {i === 0 && hoverTag && (
                <div className="absolute -top-6 left-0 bg-blue-500 text-white text-[10px] px-1.5 py-0.5 rounded-t-sm font-medium whitespace-nowrap shadow-sm pointer-events-none uppercase tracking-wider">
                  {hoverTag}
                </div>
              )}
            </div>
          ...
          {hoverBox && (
            <div
              className="absolute border-2 border-blue-500 bg-blue-500/10 rounded-sm transition-all duration-75 ease-out"
              style={{
                top: hoverBox.top,
                left: hoverBox.left,
          ...
          {focusBox && (
            <div
              className="absolute border-2 border-purple-500 bg-purple-500/5 rounded-sm z-[100]"
              style={{
                top: focusBox.top,
                left: focusBox.left,
            ...
            >
              {focusTag && (
                <div className="absolute -top-6 left-0 bg-purple-500 text-white text-[10px] px-1.5 py-0.5 rounded-t-sm font-medium whitespace-nowrap shadow-sm pointer-events-none uppercase tracking-wider">
                  {focusTag}
                </div>
              )}
              ...
              {/* Resize Handles */}
              <div
                className="resize-handle absolute -right-1 -bottom-1 w-3 h-3 bg-purple-500 border border-white rounded-full cursor-nwse-resize z-[101]"
                onMouseDown={(e) => handleResizeStart(e, "se")}
              />
              <div
                className="resize-handle absolute -right-1 top-1/2 -translate-y-1/2 w-2 h-4 bg-purple-500 border border-white rounded-sm cursor-ew-resize z-[101]"
                onMouseDown={(e) => handleResizeStart(e, "e")}
              />
              <div
                className="resize-handle absolute left-1/2 -translate-x-1/2 -bottom-1 w-4 h-2 bg-purple-500 border border-white rounded-sm cursor-ns-resize z-[101]"
